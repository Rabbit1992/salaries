// 专门用于调试工资表字段结构的脚本
// 运行命令: node debug-salary-fields.js
...
// 查看工资表字段结构
async function debugSalaryFields() {
  console.log('🔍 正在查看工资表字段结构...');
  ...
  try {
    const url = `https://open.feishu.cn/open-apis/bitable/v1/apps/${feishuConfig.APP_TOKEN}/tables/${feishuConfig.SALARY_TABLE_ID}/records?page_size=1`;
    ...
    if (data.code === 0 && data.data.items.length > 0) {
      ...
      console.log('❌ 无法获取工资表数据:', data.msg || '表格为空');
    }
  } catch (error) {
